...
export function renderProfilePage(container) {
  ...
  const purchasesHTML = user.history.purchases.length === 0 
    ...
    : user.history.purchases.map(p => `
        <div class="flex justify-between p-4 border border-surface-100 rounded-xl mb-3 shadow-sm hover:border-primary-200 transition-colors bg-surface-50 text-sm">
  ...
  const browsingHTML = user.history.browsing.length === 0
    ...
    : `
      <div class="overflow-x-auto bg-white rounded-xl border border-surface-100 shadow-sm">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-surface-50 border-b border-surface-200 text-xs uppercase tracking-wider text-surface-500 font-semibold">
                ...
                <tr class="hover:bg-surface-50 transition-colors group cursor-pointer" onclick="window.location.hash = '#/product/${product.id}'">
                  <td class="py-3 px-4">
                    <div class="flex items-center gap-3">
                      <div class="w-10 h-10 flex-shrink-0 bg-white rounded-lg border border-surface-200 flex items-center justify-center overflow-hidden p-1">
  ...
  const chatsHTML = user.history.chats.length === 0
    ...
    : user.history.chats.map(c => `
        <div class="p-4 border border-surface-200 rounded-2xl mb-4 bg-white shadow-sm">
          ...
          <div class="mt-4 pt-3 border-t border-surface-100">
             <div class="flex gap-2">
                <input type="text" placeholder="Написать сообщение..." class="flex-1 bg-surface-50 border border-surface-200 px-3 py-2 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 outline-none">
    ...
    <form onsubmit="window.handleProfileUpdate(event)" class="space-y-5 max-w-3xl">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
          ...
          <input type="text" id="profSurname" value="${user.surname || ''}" placeholder="Иванов" class="w-full px-4 py-3 border border-surface-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">
        </div>
        <div>
          <label class="block text-sm font-medium text-surface-700 mb-1.5">Имя</label>
          <input type="text" id="profName" value="${user.name || ''}" placeholder="Иван" required class="w-full px-4 py-3 border border-surface-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">
        </div>
        <div>
          <label class="block text-sm font-medium text-surface-700 mb-1.5">Отчество</label>
          <input type="text" id="profPatronymic" value="${user.patronymic || ''}" placeholder="Иванович" class="w-full px-4 py-3 border border-surface-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">
        </div>
        <div>
          <label class="block text-sm font-medium text-surface-700 mb-1.5">Телефон</label>
          <input type="tel" value="${user.phone || ''}" disabled class="w-full px-4 py-3 bg-surface-50 border border-surface-200 rounded-xl text-sm text-surface-500 cursor-not-allowed">
        </div>
        <div>
          <label class="block text-sm font-medium text-surface-700 mb-1.5">Email</label>
          <input type="email" id="profEmail" value="${user.email || ''}" placeholder="email@example.com" class="w-full px-4 py-3 border border-surface-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">
        </div>
        <div>
          <label class="block text-sm font-medium text-surface-700 mb-1.5">Год рождения</label>
          <input type="number" id="profBirthYear" min="1900" max="${new Date().getFullYear()}" value="${user.birthYear || ''}" placeholder="1990" class="w-full px-4 py-3 border border-surface-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">
        </div>
        <div>
          <label class="block text-sm font-medium text-surface-700 mb-1.5">Город проживания</label>
          <select id="profCity" class="w-full px-4 py-3 border border-surface-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">
        ...
        <div class="md:col-span-2 pt-4 border-t border-surface-100 mt-2"><h4 class="font-bold text-surface-800 text-sm">Данные для 3 уровня (Паспорт)</h4></div>
        <div>
          <label class="block text-sm font-medium text-surface-700 mb-1.5">ИНН (ПИН)</label>
          <input type="text" id="profInn" value="${user.inn || ''}" placeholder="14 цифр" class="w-full px-4 py-3 border border-surface-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">
        </div>
        <div>
          <label class="block text-sm font-medium text-surface-700 mb-1.5">Паспорт (ID)</label>
          <input type="text" id="profPassport" value="${user.passport || ''}" placeholder="ID123456" class="w-full px-4 py-3 border border-surface-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">
        </div>
      </div>
      
      <div class="mt-8 pt-6 border-t border-surface-100 flex flex-col md:flex-row items-center justify-between gap-4">
